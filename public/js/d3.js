...
function link(links) {
  ...
  function force(alpha) {
    ...
    var __key = Object.getOwnPropertyNames(nodes[0])[0];
    for (var k = 0, n = links.length; k < iterations; ++k) {
      for (var i = 0, link, source, target, x, y, l, b; i < n; ++i) {
//        link = links[i], source = link.source, target = link.target;
    	//指定字段关联link的source和targer
    	link = links[i];
    	if (typeof link.source == 'object') {
    		source = link.source;
    		target = link.target;
	    } else if (typeof link.source == 'string') {
			source = nodes.filter(function(mn){return mn[__key] == link.source;})[0];
			target = nodes.filter(function(mn){return mn[__key] == link.target;})[0];
	    } else {
  ...
  function initialize() {
    ...
    for (i = 0, count = new Array(n); i < m; ++i) {
      ...
      var __key = Object.getOwnPropertyNames(nodes[0])[0];
      if (typeof link.source !== "object") {
    	  link.source = nodes.filter(function(mn){return mn[__key] == link.source;})[0];
      }
      if (typeof link.target !== "object") {
    	  link.target = nodes.filter(function(mn){return mn[__key] == link.target;})[0];
      }
